...
def signup():
    ...
    if username and password:
        try:
            ...
            main_cursor.execute(
                f"CREATE TABLE IF NOT EXISTS {username} (dish_name TEXT, origin TEXT, preparation_time TEXT, image_url TEXT)")
...
def loginPage():
    ...
    login_label = ctk.CTkLabel(login_page, text="Meal Maker")
...
def dashboard():
    ...
    dashboard_page = ctk.CTkFrame(win, width=1000, height=800)
    dashboard_page.pack()


    dashboard_page.tkraise()  # Show the dashboard

    # Create and place the "My Meals" button
    my_meals_button = ctk.CTkButton(dashboard_page, text="My Meals", command=my_meals)
    my_meals_button.place(relx=0.5, rely=0.3, anchor=ctk.CENTER)

    # Create and place the "New Meals" button
    new_meals_button = ctk.CTkButton(dashboard_page, text="New Meals", command=new_meals_before1)
    new_meals_button.place(relx=0.5, rely=0.6, anchor=ctk.CENTER)

    # Adjust button size by configuring the style
    style = ttk.Style()
    style.configure('TButton', font=('Helvetica', 45), padding=10)  # Adjust font and padding as desired
...
def new_meals_before1():
    global pop
    pop = Toplevel(win)
    pop.title("Options")
    pop.geometry("2000x500")

    pop_label = ctk.CTkLabel(pop, text="Do you want to input your ingredients manually or by speech?")
    pop_label.pack()

    yes = ctk.CTkButton(pop, text="Manually", command=lambda: new_meals_before2())
    yes.pack(pady=5)
    no = ctk.CTkButton(pop, text="Speech", command=lambda: speech_input())
    no.pack(pady=5)


def new_meals_before2():
    global pop
...
def speech_input():
    global pop, selected_ingredients
    ...
    pop_label = ctk.CTkLabel(pop, text="Start speaking, naming the ingredients you have in our fridge")
    pop_label.pack()

    yes = ctk.CTkButton(pop, text="Start", command=start_speech_recognition)
    yes.pack(pady=5)
    no = ctk.CTkButton(pop, text="Stop", command=stop_speech)
    no.pack(pady=5)

    tick_button = ctk.CTkButton(pop, text="✓", command=lambda: update_ingredients())
    tick_button.place(relx=0.8, rely=0.8)
...
def speech():
    global is_listening, selected_ingredients
    r = sr.Recognizer()  # Initialize the speech recognizer

    while is_listening:  # Continue looping as long as is_listening is True
        try:
            with sr.Microphone() as source2:  # Use the microphone as the source
                r.adjust_for_ambient_noise(source2, duration=0.2)  # Adjust for ambient noise

                audio2 = r.listen(source2)  # Listen for the user's input
                MyText = r.recognize_google(audio2).lower()  # Use Google's service to recognize the speech

                print("In your fridge you have ", MyText)  # Print the recognized text
                selected_ingredients.append(MyText)
                # Create a label in the GUI to display the recognized text
                pop_label = ctk.CTkLabel(pop, text=f"In your fridge you have {MyText}")
                pop_label.pack()  # Add the label to the GUI


        except sr.RequestError as e:
            # Handle the exception if the request to the speech recognition service fails
            print("Could not request results; {0}".format(e))

        except sr.UnknownValueError:
            # Handle the exception if the speech is not recognized
            print("unknown error occurred")
def start_speech_recognition():
    global is_listening
    is_listening = True
    threading.Thread(target=speech).start()
def stop_speech():
    global is_listening
    is_listening = False
...
def new_meals(choice):
    global new_meals_page, dashboard_page, my_meals_page, tabview, selected_ingredients, current_user, manual
    global pop
    pop.destroy()
    manual = 1
    ...
    dairy_listbox = tk.Listbox(tabview.tab("dairy"), selectmode=tk.SINGLE, height=50, width=30, font=("Helvetica", 30))
    ...
    selected_ingredients_listbox = tk.Listbox(new_meals_page, selectmode=tk.SINGLE, height=22, width=20, font=("Helvetica", 30))
...
#meats
    ...
    meats_listbox = tk.Listbox(tabview.tab("meats"), selectmode=tk.SINGLE, height=50, width=30, font=("Helvetica", 30))
...
###########
    ...
    search_entry = tk.Entry(new_meals_page, font=("Helvetica", 30))
...
def parameters():
    ...
    global new_meals_page, parameters_page, tabview, manual
    ...
    parameters_page = ctk.CTkFrame(win,
                                   width=1200,
                                   height=700)
    parameters_page.place(relwidth=1, relheight=1)
    ...
    lb2 = ctk.CTkLabel(parameters_page, text="Parameters Page", font=("Helvetica", 30))
    ...
    my_list = tk.Listbox(listbox_frame, width=30, height=listbox_height,
                         font=("Helvetica", 20))  # Adjust font size as needed
